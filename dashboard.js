...
// All files live at the repo ROOT (same folder as index.html)
// Paths: dashboard.html, calendario.html, aulas.html, etc.
...
<path d="M343.9 104.08C340.66 98.11 336.08 93.01 330.29 88.92L318.88 82.14L324.17 78.26C327.14 76.09 331.79 70.39L335.7 63.28C338.3 54.46C339.27 43.59C339.27 34.89 336.85 27.19 332.08 20.71C327.43 14.18 320.52 9.08 311.48 5.5C304.9 2.84 288.28 0.46H195.63V180.28H263.17L347.34 112.83C345.38 106.81 343.9 104.08ZM247.11 43.85H268.48C274.63 43.85 281.61 47.71C284.34 50.1 285.73 57.23C285.73 62.65 283.92 64.7C282.89 66.59 278.52 69.07C276.13 70.1 268.99 70.62H247.11V43.85ZM292.98 129.75C291.73 132.03 287.95 134.89C285.42 136.02 279.55 136.52H247.11V107.69H278.26C284.88 108.29 287.27 109.49C291.48 112.07 292.68 114.12C294.74 121.33C294.14 127.37 292.98 129.75Z" fill="#161616"/>
<path d="M338.54 15.99C344.33 23.87 347.27 43.59L382.14 85.06L438.39 39.98L410.16 0.09H319.05C332.38 8.68 338.54 15.99Z" fill="#161616"/>
...
// ── ALL NAV ITEMS — hrefs are root-level filenames ──
const NAV = [
  { icon:'home',     label:'Início',            href:'dashboard.html',    match:'dashboard'  },
  ...
  { icon:'tcc',      label:'Meu TCC',            href:'https://tcc.mbx.academy/', match:'tcc', ext:true },
...
// Detect active page from filename (e.g. "dashboard" from "dashboard.html")
function activePage() {
...
function buildTopbar() {
  ...
  el.className = 'topbar';
  el.innerHTML = `
    <button class="tb-burger" aria-label="Menu">${IC.burger}</button>
    <a class="tb-logo" href="index.html" title="Início">${MBX_SVG}</a>
    <div class="tb-spacer"></div>
    <div class="tb-actions">
      <button class="tb-btn" title="Caixa de Entrada">${IC.inbox}</button>
      <button class="tb-btn" title="Notificações" id="tb-notif" style="position:relative">
        ${IC.bell}<span class="tb-notif-dot"></span>
      </button>
      <button class="tb-btn" title="Apoio e Dicas">${IC.help}</button>
      <div class="tb-avatar" id="tb-avatar" title="Perfil">L</div>
    </div>`;

  document.getElementById('tb-notif').addEventListener('click', e => {
    e.stopPropagation(); toggleDrop('dp-notif', e.currentTarget, buildNotif);
  });
  document.getElementById('tb-avatar').addEventListener('click', e => {
    e.stopPropagation(); toggleDrop('dp-profile', e.currentTarget, buildProfile);
  });
...
function buildSidebar() {
  ...
  let h = '<div class="sb-nav">';
  NAV.forEach(item => {
    if (item.sep) { h += '<div class="sb-divider"></div>'; return; }
    const isActive = active === item.match;
    const ext = item.ext ? ' target="_blank" rel="noopener"' : '';
    h += `<a class="sb-item${isActive?' active':''}" href="${item.href}" data-tip="${item.label}"${ext}>${IC[item.icon]||''}</a>`;
  });
  h += '</div>';
  h += `<div class="sb-bottom">
    <button class="sb-item" data-tip="Sair" onclick="doLogout()" style="border:none;cursor:pointer">${IC.logout}</button>
  </div>`;
  el.innerHTML = h;
}
...
function toggleDrop(id, anchor, buildFn) {
  ...
  p.className = 'drop-panel'; p.id = id;
  buildFn(p);
...
function buildNotif(p) {
  ...
  p.innerHTML = `
    ...
    <div style="padding:10px 18px;text-align:center"><a style="font-size:12px;color:#999;text-decoration:none;cursor:pointer">Ver todas</a></div>`;
}
...
function buildProfile(p) {
  ...
  p.innerHTML = `
    <div style="padding:16px 18px;border-bottom:1px solid #E5E5E5;display:flex;align-items:center;gap:12px">
      ...
      <div><div style="font-size:14px;font-weight:600">Luiz</div><div style="font-size:12px;color:#999">luiz@mbx.academy</div></div>
    </div>
...
document.addEventListener('DOMContentLoaded', () => { buildTopbar(); buildSidebar(); });
